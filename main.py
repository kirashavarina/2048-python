...
#for player in get_best():
 #   print(player)

pygame.init()
screen = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Kira's 2048")
...
def draw_gameover():
    ...
    text_score = font.render(f"You got {score} points", True, WHITE)
    ...
    while not make_desicion:
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                #если хотим перезапустить с таким же именем
                if event.key == pygame.K_SPACE:
                    make_desicion = True
                    init_const()
                # если хотим новое имя
                elif event.key == pygame.K_RETURN:
                    USERNAME = None
                    make_desicion = True
                    init_const()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if is_empty(arr) and is_arr_move:
                    # получаем массив с пустыми числами
                    empty = get_empty_list(arr)
                    # переставляем переменные в массиве
                    random.shuffle(empty)
                    # находим переменную и одновременно удаляем ее из пустого массива
                    random_num = empty.pop()
                    # получаем индекс из переменной
                    x, y = get_index_from_number(random_num)
                    # вставляем два или четыре на пустое место в массиве по индексу
